            ...
            where T : TextDocumentState
        {
            ...
            foreach (var documentInfosInProject in documentInfosByProjectId)
            {
                ...
                if (_projectIdToGeneratorDriverMap.ContainsKey(oldProject.Id))
                {
                    ...
                    var edits = newDocumentStatesForProject.SelectAsArray<T, PendingEdit>(s => new AdditionalFileEdit.AddtionalFileAddedEdit(new AdditionalTextWithState(s)));
            ...
            if (project.SupportsCompilation)
            {
                ...
                GeneratorDriver? driver = _projectIdToGeneratorDriverMap.ContainsKey(project.Id)
                                       ...
                                       : project.LanguageServices.CompilationFactory?.CreateGeneratorDriver(comp, project.ParseOptions)
                                        ...
                                        .WithGeneratorProviders(ImmutableArray.Create<GeneratorProvider>(new GeneratorDriver.DummyProvider()));
