    ...
    ILoggerFactory loggerFactory)
{
    ...
    // dispatch completion to the language the skipped region originated from
    // (so e.g. typing inside a half-formed C# attribute value still offers C#
    // completions, not Razor-only completions). The OriginatingLanguage field
    // on SkippedContentSyntax is set by Synchronize at the call site. When the
    // field is SyntaxKind.None (document-level skip), walk the ancestor chain
    // and use the closest CSharpCodeBlockSyntax / MarkupBlockSyntax to decide;
