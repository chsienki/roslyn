...
/// <summary>
/// Regression guards for <see cref="FormattingVisitor"/> -- the walker that
/// produces <see cref="FormattingSpan"/>s for the indentation / whitespace
/// passes -- to ensure it handles the two recovery-introduced tree shapes:
///
///   * <c>MissingToken</c> (zero width) must NOT contribute a formatting
///     span (no source position to format) and must NOT shift the span
...
///     re-indented.
///
/// Both behaviours fall out of the existing defensive guards in
/// <see cref="FormattingVisitor"/> (the <c>IsMissing</c> / <c>IsEmpty</c>
/// early-returns in <c>AddSpan</c>) and the absence of a
/// <c>VisitSkippedContent</c> override; these tests pin that behaviour so
/// future visitor refactors don't silently regress it.
/// </summary>
public class FormattingVisitorRecoveryTest
{
    ...
    public void UnclosedCodeBlock_MissingCloseBrace_DoesNotEmitZeroWidthSpan()
    {
        ...
        // The parser produces a `CSharpStatementBodySyntax` whose
        // CloseBrace's `}` token is `MissingToken(RightBrace)` at the EOF
        // position. The formatter must not emit a zero-width
    ...
    public void EmptyBoundAttribute_Onclick_MissingValueToken_DoesNotEmitZeroWidthSpan()
    {
        ...
        // Under recovery + Component file-kind the `@onclick=""`
        // attribute produces the shape:
        ...
        // attribute value () lives at the position immediately after
    ...
    public void SkippedContent_DoesNotEmitFormattingSpans()
    {
        ...
        // `foo;` ( -- `TryBalanceBlock` recovery branch
