...
///  hovering on a zero-width <c>MissingToken</c> -- the placeholder the
///  enhanced parser inserts where a syntactically required token is absent --
///  must return no hover content. The diagnostic emitted at the same
///  position already explains what is wrong; surfacing a "phantom" hover
///  there would be both misleading and useless for navigation.
/// </summary>
public class HoverFactoryRecoveryTest
{
    ...
    public async Task Hover_OnMissingToken_FallsBackToDiagnostic()
    {
        ...
        // recovery the parser produces the BDD #9 shape -- a zero-width
