...
internal class DefaultRazorIntermediateNodeLoweringPhase : RazorEnginePhaseBase, IRazorIntermediateNodeLoweringPhase
{
    ...
    private class LoweringVisitor : SyntaxWalker
    {
        ...
        public override void VisitCSharpExpressionLiteral(CSharpExpressionLiteralSyntax node)
        {
            ...
            // An empty C# expression (e.g. the right-hand side of @onclick="")
            // arrives as a single missing-identifier token under a
            // CSharpExpressionLiteral. Tag the IR token so downstream lowering
            // passes (ComponentEventHandlerLoweringPass) and codegen can
            // substitute a safe placeholder instead of emitting a hole that
            // produces CS1525. We treat any literal whose tokens are all missing
