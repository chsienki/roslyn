...
/// <summary>
/// IR-shape tests for the missing-value marker. These tests assert that
...
/// 5 codegen can substitute a safe placeholder instead of emitting a hole
/// that produces CS1525 downstream. See
/// <c>src/Razor/docs/plans/ErrorRecovery/parser-recovery.md</c>
/// and.
/// </summary>
public sealed class MissingValueMarkerLoweringTests
{
    ...
    {
        // Under recovery the parser surfaces the missing value as a single
        // MissingToken-bearing CSharpExpressionLiteral. The lowering visitor
        // tags the IR token with IsMissingValue, so downstream passes
        // (ComponentEventHandlerLoweringPass in particular) can distinguish
        // a real empty C# value from one the user simply did not write.
    ...
    public void BoundNonStringAttributeWithEmptyValue_TagsSyntheticToken()
    {
        ...
        // a bool-bound property). tags that synthetic token as a
