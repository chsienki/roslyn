...
///
/// Tests with the suffix <c>_EnhancedRecovery</c> re-parse the same corpus
/// file under the recovery-aware parser and use in-memory assertions
/// (rather than a parallel set of baselines) to pin the recovery-model
/// exit criteria for the function being exercised. See
...
public class ParserRecoveryCorpusSnapshotTests() : ParserTestBase(layer: TestProject.Layer.Compiler, validateSpanEditHandlers: true, useLegacyTokenizer: true)
{
    ...
    // ----------------------------------------------------------------
    // ParseExplicitExpressionBody tests.
    //
    // These exercise the recovery branch in `CSharpCodeParser.ParseExplicitExpressionBody`.
    // The legacy [Fact]s above continue to pin the old behaviour via the
    // existing .stree/.diag/.cspans baselines.
    //
    // Each test asserts the exit criteria:
    ...
    public void UnclosedExplicitExpression_EnhancedRecovery()
    {
        ...
        // absorbed `foo.Bar` tokens (exit criterion -- not a fat
        // `CSharpExpressionLiteral`). The skipped node carries its
        // originating language so the IDE can route features at
        ...
        // re-absorbed as `MarkupMiscAttributeContent` (exit
        // criterion #4).
    ...
    public void EmptyExplicitExpression_EnhancedRecovery()
    {
        ...
        // is never entered and the produced tree is identical to a
        // well-formed parse. Verified by the absence of diagnostics, the
        // absence of `SkippedContentSyntax`, and the presence of a real
        // (non-missing) closing `)`.
        var explicitBody = tree.Root
            .DescendantNodes()
            .OfType<CSharpExplicitExpressionBodySyntax>()
    ...
    // ----------------------------------------------------------------
    // ParseStatementBody tests.
    //
    // Exercises the recovery branch in `CSharpCodeParser.ParseStatementBody`.
    ...
    public void UnclosedCodeBlock_EnhancedRecovery()
    {
        ...
        // `MarkupMiscAttributeContent` (exit criterion #4).
        Assert.Empty(tree.Root.DescendantNodes().OfType<MarkupMiscAttributeContentSyntax>());
    }

    // ----------------------------------------------------------------
    // ParseStandardStatement tests.
    //
    ...
    // `UnclosedIfParen.razor` do NOT actually exercise
    ...
    // `ParseMethodCallOrArrayIndex` (territory).
    // `UnclosedParenInsideCodeBlock_EnhancedRecovery` constructs an
    // input that DOES exercise `TryBalanceBlock`'s recovery,
    // giving real test coverage.
    // ----------------------------------------------------------------
    ...
    public void MidStatementGarbage_EnhancedRecovery()
    {
        ...
        // so recovery path is dead code for this input.
        //
        // Parity assertions: recovery mode produces the same shape as
        // legacy (the existing `MidStatementGarbage` baseline). This
        // pins the invariant that doesn't regress well-formed
    ...
    public void UnclosedIfParen_EnhancedRecovery()
    {
        ...
        // `@if(foo bar` is exercised by `TryParseCondition`: `if` dispatches
        // to `ParseIfStatement` -> `ParseConditionalBlock` -> `TryParseCondition`,
        // and the legacy `AcceptUntil(NewLine)` would flatten the structure
        // into a single fat `CSharpStatementLiteral`.
        //
        // Assertions for `TryParseCondition`:
        //   - `Balance`'s pre-existing RZ1027 at the opening `(` is preserved.
        //   - No new RZ1046 fires (introduced no new diagnostics).
        ...
        //   - No `MarkupMiscAttributeContent` is produced (exit
        //     criterion #4).
    ...
    {
        // Synthetic input that DOES exercise `TryBalanceBlock`
        // recovery branch (the corpus files don't -- see
        ...
        // The pre-existing RZ1027 from `Balance` remains (does
        ...
        // No `MarkupMiscAttributeContent` produced (exit criterion #4).
        Assert.Empty(tree.Root.DescendantNodes().OfType<MarkupMiscAttributeContentSyntax>());
    }

    // ----------------------------------------------------------------
    // TryParseCondition tests.
    //
    //  migrates the C# control-flow keyword frames
    ...
    // already migrated `ParseStatementBody`.
    //
    //  Exit criteria:
    ...
    //   - No new RZ1046 (introduces no new diagnostics).
    //   - The surrounding markup parses cleanly without
    //     `MarkupMiscAttributeContent` wrappers.
    //
    // Corpus coverage: `UnclosedForeach.razor` and `UnclosedSwitch.razor`
    // (added) exercise the canonical conditional-block
    // recovery via `ParseConditionalBlock` + `TryParseCondition`.
    // `UnclosedIfParen_EnhancedRecovery` above (updated)
    ...
    public void UnclosedSwitch_EnhancedRecovery()
    {
        ...
        // The trailing markup elements parse cleanly: two `MarkupElement`s
        // for the `<p>one</p>` and `<p>other</p>` chunks. The
        // SkippedContent doesn't pollute the markup transition.
    ...
    public void UnclosedCatchParen_EnhancedRecovery()
    {
        ...
        //                 recovery branch syncs at the body `{`,
    ...
    public void UnclosedUsingParen_EnhancedRecovery()
    {
        ...
        //   @using(var x = foo bar { }
        //         ^^^^^^^^^^^^^^^^^
        //         Balance(BacktrackOnFailure) fails; the 
        //         recovery branch syncs at the body `{`, wrapping
    ...
    // ----------------------------------------------------------------
    // Razor directive parsers tests.
    //
    //  migrates `ParseExtensibleDirective` (the workhorse for
    ...
    //
    // Exit criteria (per plan section ""):
    ...
    //
    // Corpus coverage: `MalformedUsing.razor` (added)
    ...
    public void MalformedUsing_EnhancedRecovery()
    {
        ...
        // recovery branch is purely shape-cleanup.
        Assert.Empty(tree.Diagnostics.Where(d => d.Id == "RZ1046"));
        ...
        // ` bar` as `MarkupTextLiteral` -- recovery absorbs it
    ...
    public void MalformedInherits_EnhancedRecovery()
    {
        ...
        //
        // `BuildBailedDirective` helper wraps the bail
        ...
        // Recovery introduces no new diagnostics.
        Assert.Empty(tree.Diagnostics.Where(d => d.Id == "RZ1046"));
        ...
        // markup side. Recovery prevents leaked C# tokens
        ...
        // ` bad` as `MarkupTextLiteral` -- recovery absorbs it
        // before the outer markup parser sees it).
        Assert.DoesNotContain(
            tree.Root.DescendantNodes().OfType<MarkupTextLiteralSyntax>(),
    ...
    // ----------------------------------------------------------------
    // ParseMethodCallOrArrayIndex tests.
    //
    ...
    //   - The closing-bracket emission via `Required(right, ...)`, so the
    //     missing close bracket is always represented in the tree (either
    //     as the real token or as a zero-width `MissingToken` carrying a
    //     narrow RZ1027 diagnostic).
    //
    // Exit criteria:
    ...
    public void UnclosedMethodCallInImplicit_EnhancedRecovery()
    {
        ...
        // `Balance`'s `ErrorSink.OnError`; the recovery branch suppresses
        ...
        // tokens (exit criterion -- not a fat
        // `CSharpExpressionLiteral`). Tagged with `CSharpCodeBlock` so
        //  can route IDE features at positions inside the
        ...
        //  exit criterion #4: no `MarkupMiscAttributeContent`
    ...
    // ----------------------------------------------------------------
    // Implicit expression hitting markup -- validation test.
    //
    ...
    // emitted.
    //
    // This test is the validation case: the recovery mode tree
    // must be the same shape as the legacy one (no `Balance` failure to
    // trigger the sync, no `Required` to introduce missing
    ...
    //   - The outer-follow threading from is correctly a no-op
    ...
    public void ImplicitExpressionHittingMarkup_EnhancedRecovery()
    {
        ...
        // content, no missing brackets (the sync branch is
        ...
        // not as `MarkupMiscAttributeContent` (exit criterion #4
    ...
    // ----------------------------------------------------------------
    // ParseStartTag / ParseEndTag tests.
    //
    // Exercises the recovery branches in:
    //   - The tag-name slot in `ParseStartTag` and `ParseEndTag` via
    //     `Required(SyntaxKind.Text, ...)` -- a missing tag name is
    //     represented as a zero-width `MissingToken(Text)` with a narrow
    //     RZ1047 (Parsing_TagNameExpected) diagnostic attached at the
    //     precise cursor position;
    //   - (Indirectly, via the existing 28 corpus tests passing) the
    //     close-angle slot via `Required(SyntaxKind.CloseAngle, ...)`,
    //     emitting a narrow RZ1024 (Parsing_UnfinishedTag) on the missing
    //     close angle.
    //
    // Exit criteria:
    ...
    //     `MarkupElement` (produces no recovery contamination
    //     that would push it into `MarkupMiscAttributeContent`).
    //   - No RZ1024 diagnostics: recovery only emits
    ...
    public void UnnamedTag_EnhancedRecovery()
    {
        ...
        // not exercised here. ('s close-angle migration is
        // covered indirectly by the 28 pre-existing corpus baselines
        // continuing to pass under recovery mode.)
        ...
        // The trailing `<p>after</p>` parses as a real `MarkupElement`
        // with real (non-missing) tag-name tokens. exit
        ...
        // around the unnamed tags. ('s tag-name sync adds any
    ...
    // ----------------------------------------------------------------
    // ParseRemainingAttribute tests for the
    ...
    // separate `MarkupMiscAttributeContent` nodes -- the 
    // tag-helper rewriter glues those back together. only
    ...
    //     snapshot shows).
    //   - Its `Value` is exactly the shape: one `GenericBlockSyntax`
    ...
    //   - No new parser diagnostics are introduced by the recovery
    //     branch (RZ2008 is emitted later by tag-helper resolution, not
    //     by the parser; this assertion guards against the recovery
    ...
    public void EmptyBoundAttribute_Onclick_EnhancedRecovery()
    {
        ...
        // shape: GenericBlock([ CSharpExpressionLiteral([ MissingToken(Identifier) ]) ]).
        //  codegen detects this exact shape (single-child
        ...
        // it has a non-null `Value` with real content (only
        ...
        // The recovery branch must not emit any parser diagnostics --
        ...
        // not in the parser. This assertion guards against the recovery
        // branch accidentally widening the diagnostic set.
        Assert.Empty(tree.Diagnostics);
    ...
    // ----------------------------------------------------------------
    // TryRecoverStartTag / CompleteEndTag recovery
    // test.
    //
    ...
    //     silent path is unchanged -- see the
    ...
    public void UnclosedTag_EnhancedRecovery()
    {
        ...
        // a diagnostic emission site, so no RZ1025 / RZ1026
    ...
    // `<p>` shape too (no whitespace before `>`), so the recovery branch
    // must match the no-op behaviour to avoid contaminating clean markup
    // with a spurious RZ1048.
    //
    // Test layout:
    ...
    public void MalformedTagAttribute_EnhancedRecovery()
    {
        ...
        // (position 12). The recovery branch emits a zero-width RZ1048
        ...
        // recovery; we filter to the markup-side
        // skipped node here.
        ...
        // not part migration scope and is unchanged
        // under recovery mode.
        ...
        //  exit criterion: the `=` is no longer wrapped in a
        ...
        // and `>`). The recovery branch must NOT emit RZ1048 here.
        {
            ...
            // at the follow-set boundary (`>`), so the recovery branch
    ...
    // ----------------------------------------------------------------
    // outer-follow threading through recovery mode recovery.
    //
    //  upgrades every recovery mode `Synchronize` / `Required`
    // / `OtherParserBlock` call site added by Stages 2 and 3 to thread
    // the parser's `_outerFollow` (populated.1's
    ...
    public void MalformedCSharpWithSurroundingMarkup_EnhancedRecovery()
    {
        ...
        // (`(NewLine, LeftBrace, RightBrace)`). Without
        ...
        // No `MarkupMiscAttributeContent` produced (exit
