...
/// <summary>
/// Audit <see cref="TagHelperParseTreeRewriter"/> and
/// <see cref="TagHelperBlockRewriter"/> against tree shapes produced by the
/// recovery-aware parser.
///
/// The parser can place two shapes inside
...
///     <c>ParseMiscAttribute</c>'s recovery.
///   </description></item>
/// </list>
///
...
public class TagHelperRewriter_EnhancedRecoveryTests : TagHelperRewritingTestBase
{
    ...
    {
        // `@onclick=""` on a directive-attribute-bound tag helper
    ...
    {
        // Contract: minimized tag-helper directive attributes represent
        // "no value at all" -- distinct from "empty value". The rewriter
        // must continue to produce a
        // MarkupMinimizedTagHelperDirectiveAttributeSyntax with no value
        // subtree, even when the surrounding tree contains missing tokens
        // or skipped content.
        var (rewritten, _) = ParseAndRewrite(
            @"<input @onclick />",
            s_inputDirectiveAttribute_TagHelpers);
    ...
    {
        // Confirms that the "no value at all" path is unaffected by the
        // tree shapes produced under recovery.
        var (rewritten, _) = ParseAndRewrite(
            @"<input count />",
    ...
    {
        // `ParseMiscAttribute` migration introduces
        ...
        // The trailing `@onclick=""` retains its missing-Identifier
