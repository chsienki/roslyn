...
/// <c>Synchronize</c> calls invoke <c>EndingBlock()</c> when they stop at
/// an outer-follow token (<c>SyncStopReason.AtOuterFollowToken</c>) via
/// the <c>EndingBlockIfStoppedOnOuter</c> helper, so the wrapped Roslyn
/// <c>SyntaxTokenParser</c>'s position and state stay aligned with the
/// cursor the HTML parser resumes from. The matching <c>StartingBlock</c>
/// call already fires automatically when C# is re-entered via
/// <c>ParseBlock</c>.
///
/// Tests run with <c>UseRoslynTokenizer = true</c> (default for this class
/// -- <c>useLegacyTokenizer: false</c>). The native tokenizer's
/// <c>StartingBlock</c>/<c>EndingBlock</c> are no-ops, so the matching
/// native-tokenizer coverage already lives in
/// <see cref="ParserRecoveryCorpusSnapshotTests"/>; the third test here
/// pins the native/Roslyn tree-equality invariant explicitly.
/// </summary>
public class RoslynCSharpTokenizerRecoveryTests() : ParserTestBase(layer: TestProject.Layer.Compiler, validateSpanEditHandlers: true, useLegacyTokenizer: false)
{
    ...
    public void MalformedCSharpWithSurroundingMarkup_RoslynTokenizer_EnhancedRecovery()
    {
        ...
        // `<` token; the `EndingBlockIfStoppedOnOuter` hook
        ...
        // `SkippedContentSyntax` (invariant -- preserved here
    ...
    {
        // The behavioural correctness check for after the C#
        ...
        //
        // The `EndingBlockIfStoppedOnOuter` hook discards any
    ...
    {
        // Tokenizer choice must not be observable in the produced syntax
        // tree when both tokenizers are wired through cross-language sync.
        // `EndingBlockIfStoppedOnOuter` keeps the Roslyn tokenizer's
        // wrapped-parser state in sync with the native tokenizer's (which
        // is a no-op for these hooks) across multiple recovery + re-entry
        // cycles.
        //
        // The chosen corpus stresses several recovery sites:
        // `ParseExplicitExpressionBody` (`@(`), `TryParseCondition` (`@if(`),
        // and implicit-expression re-entry (`@x`).
        var sources = new[]
        {
